...
> **当前状态覆盖（2026-08-03）**：本文保留 P3-M0 冻结时的架构与数据契约原文；当前 P3-M1～M8 已 **PASS**，P3-M9 尚未完成，P4 尚未开始。下方“契约未实现、P3-M1 尚未开始”等表述仅记录 M0 冻结时点，不代表当前进度。
> **当前产品定位**：P3 是“已治理知识复用资产生产和交付中心”，覆盖 `training_material`、`sop`、`service_script`、`qa_bank`、`sft_dataset` 五类资产。
> **状态语义**：`approved` 只表示通过 P3 审核，不等于 `published`；`published` 只表示 P3 受治理资产已发布，不等于已进入 RAG、MCP、Agent 或模型训练；`export` 只表示生成受治理 Artifact，不等于已执行训练。
> **部署口径**：本地 Docker 的阶段验收或发布证据不等于生产部署或线上持久化验收；P3 完成口径仍须通过 P3-M9 Release Closure。
