...
P2-M8.1 local implementation, real local P2 semantic acceptance, and the sealed-P1 local Harness are complete. Unified retrieval, RRF, shadow retrieval, Agent consumption, native image/multimodal embedding, real AI extraction, Phase 3, and Phase 4 must not be implemented outside separately authorized milestones. If final M8.1.1 automated checks pass, M8.2 may proceed only as local Shadow development/testing under a separate scope; Render Shadow and online P2 enablement remain blocked until persistent storage is attached/configured and separately accepted.
