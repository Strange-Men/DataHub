...
P3-M1 Source Eligibility is complete at the local Docker release-closure boundary. P1/P2 remain formally frozen at the post-release patch baseline; CustomerOpsAgent remains P1-only by default, Unified remains explicit opt-in, Render P2 persistence remains BLOCKED, and local Docker remains the authoritative runtime boundary. P3 now has a read-only P1/P2/Bad Case correction eligibility core, bounded single/batch API and centralized `p3.source.read` RBAC. No P3 table, Project, generation, export or frontend workflow exists. P3-M2 has not started.

The cumulative P1/P2 chronology below is historical. Any older “current checkpoint” wording is superseded by this P3-M1 release status.
