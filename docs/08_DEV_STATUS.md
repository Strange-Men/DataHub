...
Current code contains the sealed P1 foundation plus isolated P2-M1 ingestion, P2-M2 extraction, P2-M3 human review, P2-M4 Knowledge Asset governance, P2-M6 index-control/text projection, P2-M7 profile-versioned text embeddings, P2-M8.1 explicit serving/P2-only retrieval, the M8.2 dual-index Shadow/RRF layer, and the additive M8.3 versioned Agent opt-in layer.

M8.3 keeps the Agent default and old endpoint P1-only. Only the new v2 endpoint plus explicit request strategy and four safe server-side gates can return active Unified evidence; degraded Unified output falls back to sealed P1. M9 adds no major feature and closes only the local Docker release. Default cutover, native image/multimodal embedding, real AI extraction, Phase 3, and Phase 4 remain outside scope. Render online P2 enablement remains blocked until persistent storage is attached/configured and separately accepted.

## P2-M8.3 Acceptance Snapshot

- Default/flag-off/flag-on-without-opt-in requests remain `customerops_vector_retrieval`.
- Explicit opt-in with Agent/Unified/P2 enabled and Shadow disabled returns `customerops_unified_retrieval` with P1+P2 governed evidence.
- Docker 50 ms branch-timeout injection falls back to P1 with a sanitized reason; all flags are restored to false afterward.
- P1 Harness: 10/10, trace `p1-harness-20260716-030109-dbcb8c`, fallback false, Bad Case PASS.
- P2 Eval: exact recall 1.0, MRR 0.95, archived leakage 0, zero failures.
- Unified Shadow Eval: candidate exact recall 1.0 versus control 0.0, candidate MRR 0.6071, source coverage 1.0, archived leakage 0, Shadow violations 0.
- Focused regression: 98 passed. Authoritative clean-export full backend suite: 379 passed. Compileall and frontend build: PASS.
- Render Deployment Acceptance remains **BLOCKED** by missing Persistent Disk; no local Docker result is described as Render online acceptance.

## P2-M9 Final Acceptance Snapshot

- Final governed P2 trace: `p2-local-20260716-034412-258b8ba0`; real SiliconFlow/Qwen3/1536; ready zero hit, explicit serve hit, archive zero hit, version replacement PASS.
- Fresh Asset `asset_7788ab58f2174634ab7c` row and physical object survived backend and PostgreSQL restart; pgvector remained 0.8.5.
- Final P1 Harness: 10/10, trace `p1-harness-20260716-034452-187174`, vector mode, fallback false, Bad Case PASS.
- Final P2 Eval: exact recall 1.0, MRR 0.525, archived leakage 0, duplicate rate 0.0, zero failures.
- Final expanded-corpus Unified Eval: candidate exact recall 0.8571 versus control 0.0, candidate MRR 0.4286, source coverage 1.0, archived leakage 0, Shadow violations 0, zero failures.
- Final Agent traces: Active `agent-opt-in-smoke-20260716-034826-d5dd31`; default-off `agent-opt-in-smoke-20260716-034915-889151`; controlled timeout fallback PASS; final flags all false.
- Post-commit gates: M8.2 29 passed, M8.3 14 passed, lifecycle 55 passed, clean-HEAD full backend 379 passed, compileall/frontend/Docker build and health PASS.
- `docs/56_P2_FINAL_LOCAL_DOCKER_RELEASE_REPORT.md` is the authoritative final report. Final commit/tag are created only after the last diff/secret audit.
...
- Added one provider-neutral `extraction_jobs` table with `pending`, `running`, `success`, `failed`, and `retrying` states, retry count, safe error, execution timestamps, provider, Asset reference, and extraction type.
- Added one versioned `asset_extractions` result table for normalized content and metadata; OCR, Caption, and metadata use the same aggregate rather than separate tables.
- Added `ExtractionService` for job creation, execution, failure handling, result persistence, state transitions, and internal retry orchestration.
- Added `ExtractionProvider`, `ExtractionContext`, and `ExtractionOutput` contracts plus a deterministic `MockExtractionProvider` that performs no image analysis or external request.
- Added `POST /api/assets/{asset_id}/extract`, `GET /api/extraction/jobs/{job_id}`, and `GET /api/assets/{asset_id}/extractions`.
- Namespaced P2 job ids as `asset_extract_job_*`; the pre-existing P1 job query remains behaviorally unchanged for all P1 ids and only dispatches that new P2 prefix to PostgreSQL.
- Added six focused tests covering API creation, pending/running/success transitions, persisted results, failed state, retry count, result versioning, and missing Asset/job behavior.
- Full clean-workspace repository suite passed 262/262; Python compile and focused P1/P2 extraction regression checks passed.
- Render P1 harness passed 10/10 with PostgreSQL/pgvector healthy, 30/30 SiliconFlow embeddings at 1536 dimensions, vector retrieval, and no fallback.
- Added `docs/43_P2_M2_EXTRACTION_FOUNDATION_REPORT.md`.

### P2-M2 Boundaries

- The only registered P2 provider is `mock`; its output is explicitly synthetic and is not OCR, Caption, metadata understanding, or Vision LLM output.
- M2 executes synchronously for deterministic MVP behavior, while persisted states and service boundaries support a later queue/worker without changing API or result models.
- Retry exists at the service layer; no public retry API, scheduler, queue, worker, timeout policy, cost accounting, or real-provider credential is added.
- Extraction results are unreviewed evidence only. They do not create knowledge candidates, embeddings, RAG chunks, RAG sync work, or Agent-visible content.
- No frontend change, P1 table change, tag, secret, uploaded binary, or local database is included.

## Completed In P2-M3

- Added one `extraction_reviews` model shared by OCR, Caption, and metadata results. Reviews copy the machine content for audit but never update the source `asset_extractions` row.
- Added `pending`, `approved`, `rejected`, and `needs_revision` states with a single legal transition from pending to one terminal decision.
- Added one `asset_review_snapshots` model. Approved review and snapshot are committed in the same transaction; rejected and needs-revision decisions never create a snapshot.
- Added versioning per Extraction for reviews and approved snapshots. A terminal review is immutable; a later decision creates a new review version and preserves every older snapshot.
- Added `ReviewService` for source validation, review creation, human edits, legal transition enforcement, and atomic approval.
- Added `POST /api/assets/{asset_id}/reviews`, `GET /api/reviews/{review_id}`, `PATCH /api/reviews/{review_id}`, and `GET /api/assets/{asset_id}/snapshots`.
- Added a minimal dark Material Center review workspace for Extraction viewing, pending-review create/resume, content editing, three decisions, and approved snapshot history.
- Added six focused tests covering create, approve, reject, needs revision, illegal transition, snapshot generation/versioning, and extraction immutability.
- P2 ingestion/extraction/review focused regression passed 19/19; full clean-workspace repository suite passed 268/268; frontend production build and Python compile passed.
- Render P1 harness passed 10/10 with PostgreSQL/pgvector healthy, 32/32 SiliconFlow embeddings at 1536 dimensions, vector retrieval, and no fallback.
- Added `docs/44_P2_M3_HUMAN_REVIEW_FOUNDATION_REPORT.md`.

### P2-M3 Boundaries

- Review snapshots are governance evidence only. They are not knowledge candidates, Knowledge Links, embeddings, RAG chunks, or Agent-visible records.
- No real OCR, Caption model, Vision LLM, provider credential, Embedding, RAG synchronization, unified retrieval, or CustomerOpsAgent call is added.
- No public pending-review queue, bulk review, RBAC, automatic approval, or complex review console is added.
- P1 tables, P1 frontend flows, retrieval contracts, and the namespaced P1/P2 extraction-job lookup remain behaviorally unchanged.
- No dependency, environment template, tag, secret, uploaded binary, or local database is included.

## Completed In P2-M4

- Added one isolated `knowledge_assets` model with unique source Snapshot, Asset ownership, governed content/type, draft-active-archived state, immutable version history, metadata, and timestamps.
- Added approved-only `KnowledgeAssetService` publication with source-chain validation, idempotent Snapshot replay, atomic next-version allocation, and prior-active auto-archive.
- Added full source trace from Knowledge Asset through Snapshot, Review, Extraction, Extraction Job, and Asset.
- Added `POST /api/snapshots/{id}/publish`, `GET /api/knowledge-assets`, `GET /api/knowledge-assets/{id}`, and `POST /api/knowledge-assets/{id}/archive`.
- Added a minimal dark Material Center Knowledge Asset section for publish, list, source/version/status display, and archive.
- Added six focused tests; full clean-workspace suite passed 274/274 and frontend production build passed.
- Final Render P1 harness passed 10/10 with PostgreSQL/pgvector healthy, 35/35 SiliconFlow embeddings at 1536 dimensions, CustomerOpsAgent vector retrieval, and no fallback.
- Added `docs/45_P2_M4_KNOWLEDGE_ASSET_FOUNDATION_REPORT.md`.

### P2-M4 Boundaries

- P2 Knowledge Assets are governance records only and remain isolated from P1 `rag_chunks`, `rag_embeddings`, retrieval, and CustomerOpsAgent behavior.
- No OCR, Caption, Vision LLM, Embedding, RAG synchronization, multimodal retrieval, Agent call, publication queue, or worker is added.
- Existing P2 Asset, Extraction, Review, and Snapshot contracts are preserved; the only M3 addition is a public read helper for Snapshot lookup.
- No dependency, environment template, secret, uploaded binary, local database, P1 schema change, migration rewrite, tag, or force push is included.

## Completed In P2-M5

- Added `docs/46_P2_M5_KNOWLEDGE_INDEX_PLANNING.md` as a documentation-only architecture gate.
- Chose physically isolated P1/P2 indexes with a future logical unified retrieval layer and parallel dual-route recall.
- Rejected direct P2 writes into sealed P1 `rag_chunks` or `rag_embeddings`.
- Defined index states, active/ready serving rules, immutable fingerprint/generation updates, archive-first withdrawal, and source trace requirements.
- Chose reviewed text bridging as the first P2 representation and deferred image/multimodal embeddings to an optional later stage.
- Reviewed future `p2_knowledge_index_entries`, `p2_knowledge_chunks`, and `p2_knowledge_embeddings` responsibilities and creation gates without adding any table.
- Preserved the existing P1-only CustomerOpsAgent endpoint; future fusion must use an additive versioned API, RRF, eval, shadow verification, and explicit Agent opt-in.
- Replanned the path as P2-M6 Index Foundation, P2-M7 Text Bridge Semantic Index, P2-M8 Unified Retrieval Gate, and optional P2-M9 Native Multimodal Retrieval.

### P2-M5 Boundaries

- Documentation only: no Python, TypeScript, database model, migration, API, dependency, environment, frontend, or test file changed.
- No P1 or P2 data write, table creation, vector rebuild, Embedding call, retrieval call, feature flag, or Agent integration was performed.
- P1 `rag_chunks`, `rag_embeddings`, `customerops_vector_retrieval`, schema, and business behavior remain unchanged.
- Candidate table names and fields remain recommendations; P2-M6 requires a separate schema ADR and explicit authorization before implementation.
- No tag, force push, secret, `.env`, uploaded binary, or local database is included.

## Completed In P2-M6

- Added isolated `p2_knowledge_index_entries` and `p2_knowledge_chunks` tables for lifecycle state and immutable deterministic text projection.
- Added `pending`, `building`, `ready`, reserved `serving`, `failed`, and `archived` states with separated sync state.
- Added `KnowledgeIndexService` for active-only validation, stable fingerprinting, idempotent entry creation, deterministic chunk generation, safe failure state, and archive.
- Added atomic Index archive propagation when a Knowledge Asset is explicitly archived or superseded by a later active version.
- Added `POST /api/knowledge-assets/{id}/index`, `GET /api/knowledge-index`, `GET /api/knowledge-index/{id}`, and `POST /api/knowledge-index/{id}/archive`.
- Added complete trace from Index Entry through Knowledge Asset, Snapshot, Review, Extraction/Job, and Asset.
- Added minimal Material Center Index status, generation/chunk count, projection-create, and archive controls without a retrieval page.
- Added eight focused tests; full clean-workspace suite passed 282/282 and frontend production build passed.
- Pre-deployment Render P1 harness passed 10/10 with PostgreSQL/pgvector healthy, 37/37 SiliconFlow embeddings at 1536 dimensions, vector retrieval, and no fallback.
- Added `docs/47_P2_M6_KNOWLEDGE_INDEX_FOUNDATION_REPORT.md`.

### P2-M6 Boundaries

- M6 index state and chunks are plain control/text data. No embedding/vector table, vector column/index, provider call, similarity search, retrieval API, RRF, or Agent use is included.
- `ready` means deterministic projection completed; it does not mean searchable. `serving` is reserved and has no M6 activation API.
- P1 `rag_chunks`, `rag_embeddings`, sync/build, embedding provider, CustomerOpsAgent endpoint, and retrieval behavior remain unchanged.
- No dependency, environment variable, secret, uploaded binary, local database, tag, or force push is included.

## Completed In P2-M7

- Added isolated `p2_knowledge_embeddings` with Chunk/Knowledge Asset ownership, text snapshot, vector, provider, model, dimension, profile, fingerprint, trace metadata, and creation time.
- Reused the existing mock/SiliconFlow `EmbeddingProvider` contract and current `EMBEDDING_*` configuration without changing P1 provider behavior.
- Added active Knowledge Asset + ready Index Entry eligibility, dimension validation, safe persisted provider errors, atomic persistence, and `ready -> serving` activation.
- Added immutable profile/fingerprint history and exact-replay no-op behavior.
- Added `POST /api/knowledge-index/{id}/embed` and `GET /api/knowledge-embeddings`; no retrieval API was added.
- Added offline product/policy/FAQ/version/archive/Caption eval fixtures and nine P2 semantic-index tests.
- M4/M6/M7 regression passed 23/23; clean-workspace full pytest passed 291/291; frontend production build passed.
- P1 online Harness passed 10/10 with trace `p1-harness-20260715-093458-fb44aa`, PostgreSQL/pgvector healthy, 39/39 SiliconFlow embeddings at 1536 dimensions, vector retrieval, and no fallback.
- Added `docs/48_P2_M7_TEXT_BRIDGE_SEMANTIC_INDEX_REPORT.md`.

### P2-M7 Boundaries

- P1 `rag_chunks`, `rag_embeddings`, sync/build logic, `/api/customer-ops-agent/retrieve`, and retrieval behavior are unchanged.
...
- Kept the Render failure as an explicit deployment limitation: without a configured persistent Asset disk, `POST /api/assets/upload` still returns HTTP 503 `ASSET_STORAGE_UNAVAILABLE` and no online serving corpus can be built.
- Configured an ignored local runtime with Asset storage at `D:/Claude_workfile/DataHub/.local-data/assets`, PostgreSQL 16, and pgvector 0.8.5; SQLite was not used as semantic-retrieval proof.
- Verified a real SiliconFlow call and persisted P2 embeddings with `Qwen/Qwen3-Embedding-4B`, 1536 dimensions, and profile `text_bridge:siliconflow:Qwen/Qwen3-Embedding-4B:1536`.
- Completed the public-API governed chain under trace `p2-local-20260716-014332-34783c6a`: Asset -> Extraction -> Review -> Snapshot -> Knowledge Asset -> Index/Chunk -> Embedding -> Ready -> explicit Serve -> P2 retrieval -> Archive.
- Proved the Entry remains `ready` after embedding, the target has zero recall before serve, is returned after explicit serve with `p2_vector_retrieval` and no P1 fallback, and has zero recall after archive while its physical embedding row remains.
- Proved old-version zero recall: superseding an active Knowledge Asset archived the old version and only the new active/serving version was eligible.
- Added a Git-ignored runtime expected-id manifest so formal local Eval can use exact Knowledge Asset/Chunk/Asset ids without committing environment-specific ids.
- Local P2 Eval passed 12 queries with `query_hit_rate@5=1.0`, `candidate_recall@5=1.0` over 10 positive id-labeled queries, `MRR=0.95` over 10, `semantic_mode_count=12`, `no_hit_count=2`, `archived_leakage_count=0`, duplicate rate `0.0`, average top-1/top-5 scores `0.712`/`0.6195`, average/p95 latency `2387.681`/`2900.276 ms`, and zero failed queries.
- Verified P2 retrieval logs use `p2_retrieval_*` ids and `p2_retrieval_v1` metadata with ids, scores, profile, latency, and trace summaries and contain no complete vectors or secrets.
- Local sealed-P1 Harness passed 10/10 with trace `p1-harness-20260715-175644-34b47c`, PostgreSQL/pgvector healthy, 2/2 SiliconFlow embeddings at 1536 dimensions with zero failures, `customerops_vector_retrieval`, no fallback, and Bad Case submit/draft PASS.
- Final closure verification passed: focused M4/M6/M7/M8.1 plus acceptance/Eval tests passed 59/59; the authoritative ignored clean-runtime full suite passed 327/327; Python compileall and the frontend production build passed.
- Added `docs/51_P2_M8_1_LOCAL_ACCEPTANCE_CLOSURE.md` without rewriting the earlier Render empty-corpus Eval in the M8.1 report.

### P2-M8.1.1 Acceptance Split and Boundaries

- **M8.1 Development Acceptance: PASS** based on the real local PostgreSQL/pgvector/SiliconFlow chain and exact-id retrieval evidence.
- **M8.1 Render Deployment Acceptance: BLOCKED** by the existing persistent-storage limitation; local proof does not establish Render persistence or restart durability.
- Local M8.2 Shadow development/testing is allowed only under a separately authorized scope now that the automated gates and P1 Harness have passed. Online P2 enablement, Render Shadow, and CustomerOpsAgent switching remain unauthorized.
- No Render infrastructure, Persistent Disk, S3/R2/OSS/MinIO adapter, Unified Retrieval, RRF, Shadow runtime, CustomerOpsAgent integration, P1 retrieval/vector change, frontend retrieval page, P3, or P4 capability is added by this closure.
- `.env`, `.local-data/`, runtime ids/manifest, local Assets, PostgreSQL data/credentials, vectors, API keys, and `DATABASE_URL` remain uncommitted; no tag is created.

## P2 Docker Foundation Runtime Acceptance

- Overnight execution planning was committed and pushed as `cbf0e3d [P2-Overnight] docs: initialize final closure execution state`.
- Added a clone-style Docker Compose foundation with PostgreSQL/pgvector, one-shot volume/database initialization, FastAPI backend, nginx-served frontend, health-gated startup, and named-volume persistence.
- PostgreSQL, backend, and frontend reached healthy status; pgvector 0.8.5 was available and 20 application tables initialized.
- Backend runs as non-root UID 10001. Secrets remain runtime-only; the backend entrypoint URL-encodes URL-reserved PostgreSQL password characters without printing the resulting `DATABASE_URL`, while Compose dotenv quoting rules still apply.
- Public upload created `asset_19c63ea6f3c746649aef`. The Asset database row and named-volume file survived backend restart and PostgreSQL restart.
- Docker P2 acceptance passed under trace `p2-local-20260715-192631-5058fcd7` using real SiliconFlow `Qwen/Qwen3-Embedding-4B`, dimension 1536, and the expected text-bridge profile.
- The Docker chain proved embed-keeps-ready, pre-Serve zero recall, explicit-Serve retrieval, post-Archive zero recall, and superseded-version zero recall.
- Docker P2 Eval passed all 12 queries: `hit_rate@5=1.0`, `query_hit_rate@5=1.0`, `candidate_recall@5=1.0`, `MRR=0.95`, semantic count 12, no-hit count 2, archive leakage 0, duplicate rate 0.0, top-1/top-5 averages 0.712/0.6195, average/p95 latency 296.388/373.805 ms, and zero failed queries.
- Docker sealed-P1 Harness passed 10/10 under trace `p1-harness-20260715-192652-456d0f`; the legacy CustomerOpsAgent mode and P1 physical index remain unchanged.
- README now documents safe clone/start, environment configuration, Mock versus real SiliconFlow evidence, health/init/upload, container-local acceptance/Eval/Harness, logs, persistence, cleanup, troubleshooting, security, and the Render boundary.

### Docker Foundation Closure State

- **Local Docker Runtime Acceptance: PASS**.
- Docker-focused tests passed 9/9 in 0.54 s.
- The authoritative clean-runtime backend suite passed 336/336 with 44 existing warnings in 119.35 s.
- Python compileall and the frontend production build passed.
- Diff, ignored-data, and security audits passed; `.env` and `.local-data/` remain ignored and no out-of-scope file or secret is present.
- **Docker Foundation implementation/acceptance: COMPLETED**. Commit `64c95c0 [P2-Docker] chore: add reproducible local docker environment` is pushed and `main` was synchronized with `origin/main` at the M8.2 entry boundary.
- **Render Deployment Acceptance: BLOCKED** by the existing missing Persistent Disk; the Docker evidence is not described as Render online evidence.
- No Unified Retrieval, RRF, Shadow, Agent opt-in, native multimodal embedding, object-storage cloud adapter, P3, or P4 capability is included in Docker Foundation.

## P2-M8.2 Unified Retrieval Shadow Gate

- Added additive `POST /api/v2/retrieval/search` without replacing the sealed P1 endpoint or changing the CustomerOpsAgent default path.
- Kept P1 and P2 in separate physical indexes and added independent parallel adapters through one bounded shared pool of 8 workers/16 queued-or-running tasks with an 8-second branch timeout.
- Added rank-only RRF with default `k=60`; route-local cosine scores remain diagnostics and are never compared across indexes.
- Added source-aware deduplication, current P2 Knowledge Asset/version enforcement, a default P2 per-Asset chunk quota of 2, and a fresh P2 archive/fingerprint/trace gate before response.
- Added default-off `UNIFIED_RETRIEVAL_ENABLED`, `P2_RETRIEVAL_ENABLED`, and `UNIFIED_RETRIEVAL_SHADOW_MODE` flags.
- Server-forced Shadow keeps P1 as visible control; the unified candidate is comparison-only and cannot change the returned control evidence or legacy CustomerOpsAgent behavior.
- Reused `retrieval_logs.metadata_json` under `unified_retrieval_v1`, with unified and native branch ids, latency/source/fallback/comparison metadata, and no complete vectors or secrets.
- Real Docker Unified Eval completed 11/11 queries with candidate exact recall 1.0 over 7 labeled positives versus control 0.0, candidate MRR 0.6071 versus control 0.0, candidate query hit 1.0 versus control 0.4444, source coverage 1.0, archive leakage 0 across 3 exact-labeled queries, duplicate rate 0.0, fallback 0, Shadow control violations 0, and failed queries 0.
- The Eval candidate source distribution was P1=18/P2=21; average/p50/p95 latency was 478.579/390.858/1226.990 ms, branch averages were P1=370.848 and P2=370.794 ms, and average fusion cost was 0.077 ms.
- No-answer samples returned two low-confidence results because M8.2 does not add a calibrated refusal threshold; this is retained as a known boundary.
- The P2-only baseline remains exact recall 1.0, MRR 0.95, and archive leakage 0.
- Final default-off sealed P1 Harness passed 10/10 under trace `p1-harness-20260715-201239-f5d993`; vector retrieval stayed active, fallback remained false, and `customerops_vector_retrieval` remains the control mode.
- Branch-failure isolation and duplicate/quota pressure are proven through deterministic injected unit tests, not described as live HTTP failures.
- Focused regression matrix passed 81 tests; the final M8.2 targeted suite passed 28 tests, including the explicit no-fusion path without double-counting it into the matrix.
- The authoritative clean-runtime backend suite passed 365/365 with 44 existing warnings in 251.11 seconds.
- Python compileall, frontend production build, and Docker Compose config/build/healthy-up checks passed.
- With all three Unified flags at their default `false`, the additive API failed closed with HTTP 503 while preserving retrieval/request ids; the old P1 endpoint remained healthy.
- Added `docs/54_P2_M8_2_UNIFIED_RETRIEVAL_SHADOW_REPORT.md`.

### P2-M8.2 Closure State and Boundaries

- **Local Docker Shadow runtime/Eval gate: PASS**.
- **M8.2 implementation and acceptance: COMPLETED**. Only the final staged diff/ignored-data/secret audit, phase commit, and push remain; no uncreated M8.2 commit hash is recorded.
- The existing P1 endpoint, P1 `rag_chunks`/`rag_embeddings`, P1 response contract, and CustomerOpsAgent default behavior are unchanged.
...
- **Decision: PASS and FROZEN.** `docs/71`～`docs/74` freeze P3 product scope, seven independent tables, three separate state machines, source eligibility, source invalidation, five-role permissions, deterministic generation, optional LLM draft generation, structured export and M1～M9 gates.
- P1 sources require an approved Review plus an immutable review snapshot/content fingerprint; drifted or unreviewed Candidates are ineligible.
- P2 sources require approved + active/current/non-archived Knowledge Assets with complete Snapshot/Review/version trace. Ready-not-Serving is eligible because Serving is a retrieval gate, not a P3 governance gate.
- Raw Bad Cases are ineligible; only their approved correction knowledge can enter P3.
- P3 v1 forbids chained P3 reuse, adds no independent retrieval index, runs no model training, and never writes back to P1/P2.
- Database planning follows the current additive SQLAlchemy `create_all(checkfirst)` mechanism. Application rollback retains the new P3 tables; no production destructive down migration is planned.
- Artifact retention is logical: no automatic TTL or physical deletion; admin may revoke while preserving Source Trace, Manifest and checksum.
- This checkpoint changes documentation only. No code, schema, API, frontend, test or Docker action is included.
- At the P3-M0 planning-freeze boundary, P3-M1 remained not started and required a separate explicit instruction.

## P3-M1 Source Eligibility Release Closure

- **Decision: PASS.** M1.1 provides the deterministic read-only eligibility core; M1.2 exposes protected single/batch API; M1.3 completes code inspection, regression, Docker Smoke and release evidence.
- Legal sources are `P1_KNOWLEDGE`, `P2_KNOWLEDGE_ASSET` and `APPROVED_BAD_CASE_CORRECTION`; raw Bad Cases return `RAW_BAD_CASE_NOT_ALLOWED`.
- P1 requires approved Candidate/Review, immutable review snapshot, complete Trace and matching normalized content fingerprint.
- P2 requires approved + active/current + complete Snapshot/Review/Extraction/Asset lineage. `ready` and `serving` are both eligible because retrieval visibility does not control governance reuse.
- `POST /api/p3/source-eligibility/check` and `/check-batch` are read-only; batch size is 1～100 and preserves input order.
- Central Permission `p3.source.read` is granted to admin, cleaner, reviewer, viewer and service. disabled compatibility, token 401, centralized 403, public Health and `/api/auth/me` behavior passed.
- Focused gates passed: M1.1 26, M1.2 30, Auth/RBAC 24, P1 related 2 and P2 related 6.
- Authoritative clean-export backend passed **520 tests with 5 explicit PostgreSQL skips and 44 existing warnings**. compileall, Secret scan, diff check and live OpenAPI registration passed.
- Docker Smoke passed P1 eligible/ineligible, P2 serving/archived, raw Bad Case, mixed batch, empty/over-limit validation and token-mode role access. The existing volume had no ready row, so the ready-not-serving branch was validated by the focused SQLAlchemy database test without mutating P2 state.
- No confirmed defect was found; no code repair was required. P1/P2 business code and schema remain unchanged; no P3 table exists.
- Report: `docs/75_P3_M1_SOURCE_ELIGIBILITY_RELEASE_REPORT.md`.
- P3-M2 has not started and requires a separate explicit instruction after the M1 release commit/tag are pushed.

## P3-M2 Reuse Project and Governed Source Release Closure

- **Decision: PASS.** M2.1 adds only `reuse_projects` and `reuse_source_items`; M2.2 adds deterministic repositories; M2.3 centralizes Project lifecycle and source revalidation; M2.4 exposes the governed API and RBAC.
- Project lifecycle is `draft -> active -> archived`. Only draft projects can change metadata or source selection; active freezes selection; archived is terminal and retained.
- Every selected source is rechecked by the M1.1 truth source before persistence. The Service stores the returned version, fingerprint, policy, Review/Snapshot/Knowledge Asset lineage and immutable Source Trace; callers cannot supply governance state.
- Revalidation preserves saved evidence. Ineligible, fingerprint/version/Review/Snapshot/Knowledge Asset or lineage drift marks `source_stale`; it never silently replaces a source or writes back to P1/P2.
- Activation requires at least one non-removed source, no stale source and a fresh successful eligibility check for every current source. Failure leaves the Project in draft.
- APIs cover project create/list/get/update/activate/archive and source add/list/get/logical-remove/single/bounded-batch revalidation. Batch revalidation is limited to 100.
- Central permissions are `p3.project.read`, `p3.project.write`, `p3.source.manage`, `p3.project.activate` and `p3.project.archive`. All five roles can read; cleaner can create/manage/activate; archive remains admin-only.
- Disabled-mode Docker API Smoke passed Project idempotency, governed P1/P2 source selection, archived-source rejection, list/revalidation, activation freeze, archive terminality and exact cleanup. Token-mode Smoke passed missing/invalid 401, viewer write 403 and five-role read 200; default Auth was restored to disabled.
- The development volume had no active ready-only P2 row. It was not mutated to fabricate one; ready-not-serving eligibility remains covered by the isolated M1/M2 SQLAlchemy regressions.
- Isolated PostgreSQL passed 3 integration tests for normalized uniqueness, foreign keys, idempotency, rollback and activation failure atomicity. Test containers, network and volumes were removed.
- Authoritative clean-export backend passed **653 tests with 8 explicit skips and 44 existing warnings**. M2.4 route tests passed 15; affected M2/M1/Auth regression passed 198 with 3 skips; compileall, Secret scan and diff check passed.
- P1/P2 business code, schema and source records were unchanged. No Provider, Embedding, Retrieval or Agent path was called, and no physical delete exists in the business API.
- Report: `docs/76_P3_M2_PROJECT_SOURCE_RELEASE_REPORT.md`.
- P3-M3 has not started and requires a separate explicit instruction after the M2 release commit/tag are pushed.

## P3-M3.1 Draft Asset Schema Foundation

- **Decision: PASS.** Added exactly `reuse_asset_versions` and `reuse_asset_version_sources` through the existing additive SQLAlchemy `Base.metadata.create_all(checkfirst)` path.
- `reuse_asset_versions` stores Project/type/version identity, full frozen lifecycle status, deterministic template mode, template identity, JSON content payload, content/source hashes, idempotency/audit fields and optional lifecycle/failure timestamps.
- Asset types are `training_material`, `sop`, `service_script`, `qa_bank` and `sft_dataset`.
- Version states are `generating`, `generated`, `pending_review`, `needs_revision`, `approved`, `published`, `rejected`, `failed`, `superseded` and `archived`.
- M3.1 accepts only `deterministic_template`. `llm_draft` is reserved but deliberately absent from the runtime enum and rejected by database/model validation.
- `reuse_asset_version_sources` binds one version to one selected Source Item and freezes source identity, version, fingerprint, Review/Snapshot/Knowledge Asset IDs, lineage hash and Source Trace snapshot.
- Project, Asset Version and Source Item foreign keys use `ON DELETE RESTRICT`. A Project/type/version and version/source pair are unique; asset-version idempotency key is unique.
- Later `source_stale`, logical removal or source fingerprint change does not rewrite an already-bound version snapshot.
- SQLite focused tests passed 35 with one explicit PostgreSQL skip. Key M2 Model/Repository/Service and M1 core regression passed 144 with 3 explicit PostgreSQL skips.
- Isolated PostgreSQL passed 4 integration tests, including M3.1 foreign keys/uniqueness/RESTRICT plus M2 constraints and activation atomicity. Its container, network and volume were removed.
- No full backend pytest was repeated: the single authoritative full run for this goal already passed at the M2 release boundary (`653 passed, 8 skipped`), and M3.1 changed only the scoped P3 model/test registration.
- No P1/P2 schema or data changed. No `reuse_reviews`, `export_jobs` or `export_artifacts` table exists. No Provider, generator, Repository, Service, API, frontend, review, publish or export behavior was added.
- Detail: `docs/77_P3_M3_1_DRAFT_ASSET_SCHEMA_FOUNDATION.md`.
- P3-M3 is not complete. P3-M3.2 has not started and requires separate explicit authorization.

## P3-M3 Deterministic Draft Asset Generation Release

- **Decision: PASS.** M3.2 adds bounded Repository persistence with finite PostgreSQL version-allocation retry and atomic source snapshots; M3.3 adds governed material reading, five deterministic v1 templates and synchronous generation; M3.4 adds four Asset APIs plus `p3.asset.read`/`p3.asset.generate`.
- Only active projects with 1～100 freshly revalidated, non-stale sources can create drafts. M1.1 remains the sole eligibility truth and the reader only consumes approved P1 Review snapshots or approved/current P2 Knowledge Assets.
- Asset types are `training_material`, `sop`, `service_script`, `qa_bank`, `sft_dataset`. Canonical Manifest and Content SHA-256 are stable across source input order and idempotent retries.
- Generation persists `generating -> generated|failed`. `generated` is not approved/published; no editing, review, publish, export, frontend or LLM capability was added.
- Docker Smoke passed five asset types, idempotency, new version/hash, complete P1/P2 snapshots, stale blocking, token 401/403/200 and disabled compatibility. Exact P3 smoke rows were removed; P1/P2 counts remained `36/10/104/104`.
- Isolated PostgreSQL release acceptance passed 2 tests for concurrency, idempotency, atomic snapshots, generated/failed persistence, rollback and stale-history immutability.
- Authoritative ignored clean-export backend passed **769 tests with 11 explicit skips and 44 existing warnings** in 92.08 seconds. Test containers/network/volumes and clean-export files were removed.
...
- P3-M7 started only after the dedicated Goal and is now closed below.

## P3-M7 Governed JSONL and CSV Export Release

- Status: **COMPLETE / PASS**, pending the dedicated release commit/tag in this closure turn.
- M7.1 adds only `export_jobs` and `export_artifacts`, completing the frozen seven-table model with RESTRICT foreign keys, unique create/revoke idempotency keys, one Artifact per Job and no product physical-delete flow.
- Local Storage uses root-confined relative keys, symlink/path-traversal checks and temporary-file plus atomic replace. Only incomplete failed files can be cleaned; successful Artifact files are retained.
- M7.2 exports all five Asset Types deterministically to UTF-8/no-BOM JSONL or UTF-8-BOM CSV with fixed row/column contracts, Artifact SHA-256 and Canonical `p3-export-v1` Manifest Hash.
- New export requires admin, active Project, current published Asset, approved Review, matching Hash/Manifest/Grounding and freshly valid sources. M2～M6 remain the sole governance rule sources.
- Job lifecycle is pending/running/succeeded/failed/revoked. Create and revoke are idempotent; concurrent PostgreSQL same-key creation persists one Job; Job/Artifact completion and revoke are atomic.
- M7.3 exposes create/list/detail/metadata/download/revoke through Export Service only. Central permissions are `p3.export.read/create/download/revoke`; admin writes, all five roles read/download.
- Download accepts only Artifact ID and rechecks backend/key/file/size/SHA. Public responses exclude storage key/root/absolute path, Token, full Source Trace and database details.
- Revoke returns 410 for subsequent download while preserving file and audit. Source stale blocks new export without rewriting history; historical unrevoked Artifact remains downloadable with explicit stale/current flags.
- Docker public API Smoke passed disabled/token Auth, JSONL/CSV hashes and formats, idempotency, five-role reads/downloads, revoke retention and stale-history policy. Exact smoke rows/files were removed without resetting development data or volumes.
