...
M8.3 keeps the Agent default and old endpoint P1-only. Only the new v2 endpoint plus explicit request strategy and four safe server-side gates can return active Unified evidence; degraded Unified output falls back to sealed P1. Default cutover, native image/multimodal embedding, real AI extraction, Phase 3, and Phase 4 remain outside scope. M9 may begin only after the M8.3 commit/push; Render online P2 enablement remains blocked until persistent storage is attached/configured and separately accepted.
