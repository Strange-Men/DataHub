...
P3-M3 Deterministic Draft Asset Generation is released. P1/P2 remain formally frozen at the post-release patch baseline; CustomerOpsAgent remains P1-only by default, Unified remains explicit opt-in, Render P2 persistence remains BLOCKED, and local Docker remains the authoritative runtime boundary. P3 still has exactly four additive tables and now supports governed content reading, five deterministic v1 templates, version/source-snapshot repositories, synchronous generation Service, four protected APIs and centralized asset RBAC. Results stop at `generated`; M4 LLM, review, publish, export and frontend have not started.

The cumulative P1/P2 chronology below is historical. Any older “current checkpoint” wording is superseded by this P3-M3 release status.
