...
except ModuleNotFoundError:  # imported as scripts.run_p2_rag_eval in tests
    ...
    *,
    ...
    if expected_manifest is not None:
        ...
        if run_scope is not None and isolate_run_scope:
            scope_ids = collect_p2_scope_ids(manifest_entries)
            if not scope_ids:
                raise ValueError("Scoped expected manifest contains no P2 identifiers.")
    ...
    for item in queries:
        ...
        try:
            request_payload: dict[str, object] = {
                    ...
                    "debug": verbose,
                    "request_id": f"p2-eval-{request_scope}-{query_id}",
                }
            if run_scope is not None and isolate_run_scope:
        ...
        if scope_ids:
            scoped_results = filter_p2_results(
                raw_results, scope_ids, keep_non_p2=False
            )
            scope_filtered_result_count += len(raw_results) - len(scoped_results)
            results = scoped_results[:top_k]
        else:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use the legacy global-corpus metric view even when a scoped manifest is supplied.",
    )
